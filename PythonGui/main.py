...
def handleVoiceOperation(transcriptedWords =  []):
...
def handleMetadata(text):
    ...
    text = handleVoiceOperation(transcriptedWords)
    return text
...
#        fs_new, audio = convert_samplerate(audioPath, desired_sample_rate)
#    else:
...
#    print(metadata_json_output(model.sttWithMetadata(audio, 3)))
    ...
    for frame in frames:
        if frame is not None:
            ...
            if ARGS.savewav:
                ...
                wav_data = bytearray()
            text = handleMetadata(stream_context.finishStreamWithMetadata(50))
            if ARGS.keyboard:
...
if __name__ == '__main__':
    ...
    print(pyaudio.PyAudio.get_default_input_device_info(pyaudio.PyAudio()))
